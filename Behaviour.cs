...
class SmithBehaviour : NPCBehaviour
{
  ...
  static string PickBark(Mob smith, GameState gs)
  {
    ...
    {
      List<Item> items = [];
      foreach (var slot in smith.Inventory.UsedSlots())
      {
        var (i, _) = smith.Inventory.ItemAt(slot);
        if (i is not null)
          items.Add(i);
      }
  ...
  static string Blurb(GameState gs)
  {
    ...
    if (gs.FactDb.FactCheck("SmithHammerReturned") is null)
    {
      ...
      blurbs.Add($"I think {monsters.Pluralize()} stole my hammer while I was at the tavern. How rude!");
...
class AlchemistBehaviour : NPCBehaviour
{
  ...
  public override void RefreshShop(Actor npc, GameState gs) 
  {
    ...
    for (int j = 0; j < newStock; j++)
    {      
      ItemNames itemName = gs.Rng.Next(11) switch
      {
