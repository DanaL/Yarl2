...
class TownBuilder
{
  ...
  static bool BuildingFits(Map map, int nwRow, int nwCol, Template t)
  {
    for (int r = 0; r < t.Height; r++)
    {
      for (int c = 0; c < t.Width; c++)
  ...
  void DrawPathsInTown(Map map, Rng rng, int townRow, int townCol)
  {
      ...
      for (int c = townCol; c < townCol + TOWN_WIDTH; c++)
      {
        if (map.TileAt(r, c).Type == TileType.ClosedDoor)
        {
          foreach (var adj in Util.Adj4Sqs(r, c))
          {
            ...
            if (tile.Type == TileType.Grass || tile.Type == TileType.Sand || tile.IsTree())
            {
    ...
    Loc centre = Town.TownSquare.ToList()[j];
    DijkstraMap dmap = new(map, [], Constants.WILDERNESS_WIDTH, Constants.WILDERNESS_WIDTH, true);
  ...
  bool DrawWitchesCottage(Map map, int r, int c, int townCentreRow, int townCentreCol, Template template, GameObjectDB objDb, Rng rng)
  {
    ...
    var gardenPath = AStar.FindPath(objDb, map, frontDoorLoc, gardenSq, DijkstraMap.CostWithDoors, false);
